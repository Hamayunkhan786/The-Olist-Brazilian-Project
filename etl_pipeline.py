...
print("1. Extracting Relational Data (Orders & Items) from PostgreSQL...")
orders_df = pd.read_sql_query(
    "SELECT order_id, customer_id, order_status, order_purchase_timestamp FROM source.orders;",
    engine
)

print("2. Extracting & Aggregating Reviews from MongoDB...")
...
print("3. Joining PostgreSQL Orders with MongoDB Reviews...")
enriched_df = pd.merge(orders_df, reviews_summary, on="order_id", how="left")

# Data Cleaning: Fill missing review scores with overall average or flags
enriched_df["avg_review_score"] = enriched_df["avg_review_score"].fillna(0)
enriched_df["review_count"] = enriched_df["review_count"].fillna(0).astype(int)
enriched_df["has_comment"] = enriched_df["has_comment"].fillna(False)

print("4. Writing Enriched Analytics Dataset back to PostgreSQL...")
